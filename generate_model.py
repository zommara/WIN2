...
Génère une méduse 3D animée dans les deux formats AR, à partir d'UNE seule description :
  ...
  assets/meduse-v3.glb   -> Android (WebXR / Scene Viewer) et aperçu 3D dans la page
  assets/meduse-v3.usdz  -> iPhone / iPad (AR Quick Look, modes « Objet » et « AR »)

L'animation est faite de transformations d'objets (translation, rotation, échelle) sur une hiérarchie
de pièces rigides : c'est le type d'animation que AR Quick Look lit de la façon la plus fiable
(contrairement à l'animation de squelette UsdSkel, plus capricieuse).
  - les tentacules sont des chaînes de 4 capsules articulées ; l'ombrelle pulse (échelle) ; le corps flotte ;
  - USD : temps entiers, une clé par image (24 i/s), boucle unique de 3 s (< 10 s : Quick Look la relance seul),
          pas de métadonnée exotique ;
  - glTF : les mêmes animations sur les mêmes nœuds.
