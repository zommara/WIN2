...
| `assets/meduse.glb` | Modèle avec squelette et animation intégrée, pour Android (WebXR, Scene Viewer) et l'aperçu 3D |
| `assets/meduse.usdz` | Le même modèle, même squelette et même animation, pour l'iPhone (AR Quick Look) |
...
- iPhone / iPad / Safari : AR Quick Look avec `meduse.usdz`.
...
- `meduse.glb` : squelette de 20 os (`skin`) et animation « Nage » de 3 s (`animation`), lue par model-viewer,
  par la RA WebXR de Chrome et par Scene Viewer.
- `meduse.usdz` : le même squelette et la même animation au format USD (`UsdSkel`), le format que
  AR Quick Look lit nativement sur iPhone. La boucle de 3 s y est répétée 20 fois bout à bout (1 minute),
  au cas où Quick Look ne la relancerait pas de lui-même.

Les deux animations sont générées à partir des mêmes données par `generate_model.py`.

## Personnaliser

- Autre objet : remplacez `assets/meduse.glb` (avec animation) et `assets/meduse.usdz`, ou modifiez le
  squelette, la forme et l'animation dans `generate_model.py` puis relancez-le.
- Si vous remplacez les fichiers, gardez un nouveau nom (par exemple `meduse-v2.*`) et mettez à jour `index.html` :
